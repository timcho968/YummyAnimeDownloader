...
## Необязательный токен API

YummyAnime API работает без токена для базовых запросов. Если хотите использовать токен:

1. Создайте файл `config.json` в корне проекта:
```json
{
    "yummy_app_token": "ВАШ_ТОКЕН"
}
```

2. Или задайте переменную окружения `YUMMY_APP_TOKEN`.
